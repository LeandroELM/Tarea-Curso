...
namespace Tarea_Curso_Nomina
{
    public partial class Empleados : Form
    {
        ...
        private void BuscarEmpleadoPorCodigo(string codigoEmpleado)
        {
        ...
        {
            BuscarEmpleadoPorCodigo(txtbuscarempleado.Text);
        }
        ...
        private void btnbuscarempleado_Click(object sender, EventArgs e)
        {
            ...
            {
                // Si el TextBox está vacío, restaura el color de fondo predeterminado en todas las filas
            ...
            {
                string codigoBuscado = txtbuscarempleado.Text;

                foreach (DataGridViewRow row in dvgdatos.Rows)
                {
                    string valorEnCelda = row.Cells[0].Value != null ? row.Cells[0].Value.ToString() : string.Empty;

                    if (valorEnCelda.Equals(codigoBuscado, StringComparison.OrdinalIgnoreCase))
                    {

                        foreach (DataGridViewRow r in dvgdatos.Rows)
                        {
